...
class FlippingBook implements FlippingBookInterface {
  ...
  public function prepareExportDirectory(File $file, $export_location) {
    ...
    $this->fileSystem->prepareDirectory($export_location, FileSystemInterface::CREATE_DIRECTORY);
    $destination = $this->fileSystem->getDestinationFilename($export_location . '/' . $dir, FileSystemInterface::EXISTS_RENAME);
  ...
  public function deleteArchive(FlippingBookEntity $flippingBook) {
    ...
    $this->fileSystem->deleteRecursive($this->fileSystem->realpath("$location/$dir"));
  }
